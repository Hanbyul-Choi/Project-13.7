...
function IdeaDetail({ item }: { item: Suggestion }) {
  ...
  const { session: curUser } = useSessionStore();
  ...
  return (
    <>
      {/* 챌린지 아이디어 타이틀 */}
      <h4>{title}</h4>
      {curUser?.user_id === user_id ? <DropDownBtn editClickHandler={handleAppendParamMovePage} deleteClickHandler={handleDeleteChallengeIdeaData} position={'top-[175px] right-20'} /> : <></>}
      <div className="mt-6 flex items-center justify-between">
        <div className="flex items-center">
          <div>
            <img src={curUser?.profile_img || defaultProfileImg} width={70} height={70} alt="Profile Image" className="mr-[16px] shadow-[0_1px_5px_0_rgba(53,60,73,0.08)] rounded-lg " />
          </div>
          <div>
